...
bool startCamera() {
  ...
  if (result != ESP_OK) {
    ...
    Serial.println("NodeMCU-32S may upload this binary, but it does not verify the camera pinout and is not the supported project profile.");
    Serial.println("Verify the Inland board silk-screen/pinout, regulated 5 V power, and camera ribbon orientation.");
    return false;
  }
  return true;
}
