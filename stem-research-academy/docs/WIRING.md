...
| --- | --- | ---: | ---: |
| Front left | Driver 1 IN1 / IN2 | GPIO 6 | GPIO 5 |
...
`tests/test_motor.py`. The two left-side pairs are polarity-swapped in software
for the installed motor orientation; the right side is unchanged. Do not
connect one GPIO to more than one driver input: independent channels are
required for mecanum strafe and rotation.
